...
func TestReconcileWorkspaceConfigMap(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Owned",
			workspace: testobj.Workspace("", "workspace-1", testobj.WithBackendType("gcs")),
			assertions: func(configmap *corev1.ConfigMap) {
				assert.Equal(t, "Workspace", configmap.OwnerReferences[0].Kind)
				assert.Equal(t, "workspace-1", configmap.OwnerReferences[0].Name)
			},
		},
	}
	for _, tt := range tests {
