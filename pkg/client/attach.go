...
// process.
func (c *client) Attach(pod *corev1.Pod, container, magicString string, in *os.File, out io.Writer) error {
	cfg := c.config
	...
	go func() {
		// Blocks until read by the runner on other end of the stream
	...
	exec, err := remotecommand.NewSPDYExecutor(cfg, "POST", makeAttachRequest(client, pod, container).URL())
	if err != nil {
		return err
	}
