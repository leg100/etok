...
func main() {
	for k, v := range crdinfo.Inventory {
		...
		util.GenerateTemplate(v, controller, fmt.Sprintf("%s/%s_controller.go", strcase.ToSnake(k), strcase.ToSnake(k)))
	}
}
...
import (
		{{ .Name | ToLowerCamel }}Controller "github.com/leg100/stok/pkg/controller/{{ .Name | ToSnake }}"
)
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	// Watch for changes to primary resource {{ .Name | ToCamel }}
	err = c.Watch(&source.Kind{Type: &v1alpha1.{{ .Name | ToCamel }}{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}

	// Watch for changes to secondary resource Pods and requeue the owner {{ .Name | ToCamel }}
	err = c.Watch(&source.Kind{Type: &corev1.Pod{}}, &handler.EnqueueRequestForOwner{
		IsController: true,
		OwnerType:    &v1alpha1.{{ .Name | ToCamel }}{},
	})
	if err != nil {
		return err
	}

	// Watch for changes to resource Workspace and requeue the associated {{ .Name | ToCamel }}s
...
// Reconcile{{ .Name | ToCamel }} reconciles a {{ .Name | ToCamel }} object
...
// Reconcile reads that state of the cluster for a {{ .Name | ToCamel }} object and makes changes based on the state read
// and what is in the {{ .Name | ToCamel }}.Spec
// Note:
...
func (r *Reconcile{{ .Name | ToCamel }}) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	reqLogger.V(1).Info("Reconciling {{ .Name | ToCamel }}")

	return command.Reconcile(r.client, r.scheme, request, &v1alpha1.{{ .Name | ToCamel }}{}, "{{ .Name }}")
}
`
